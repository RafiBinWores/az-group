<x-layouts.app>
    ...
    <div class="bg-white shadow-sm w-full rounded-lg mt-4">
        ...
        <div class="p-6 font-ibm">
            <form id="cutting-form" action="{{ route('cutting.update', $cutting->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $cutting->date) }}"
                        class="w-full border mt-3 outline-[#99c041] border-gray-300 px-3 py-2 rounded-xl focus:ring-[#99c041] focus:border-[#99c041] transition">
                    <span class="error text-red-500 text-xs mt-1 block"></span>
                </div>
                <!-- Buttons -->
        ...
        <script>
            ...
            function populateCuttingFields(colorQty, cuttingRows) {
                ...
                (colorQty || []).forEach((row, idx) => {
                    ...
                    <input type="number" readonly min="0" placeholder="Order Qty" value="${row.order_qty ?? (matching ? matching.order_qty : '')}" class="border border-gray-300 bg-gray-100 rounded-xl px-3 py-2 w-1/6" name="cutting[${idx}][order_qty]">
                    <input type="number" min="0" placeholder="Cutting Qty" value="${matching ? matching.cutting_qty : ''}" class="border border-gray-300 rounded-xl px-3 py-2 w-2/6" name="cutting[${idx}][cutting_qty]">
                `;
                            return div;
                        })(),
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                let selectedGarment = @json($cutting->garment_type ?? '');
                let cuttingRows = @json($cutting->cutting ?? []);
